...
/**
 * The struct Coordinate represents a position with row, column, and turn information in C++.
 * @property {int} Row - Row is an integer property that represents the row coordinate of a location.
 */
struct Coordinate
{
...
/**
 * The above code defines a struct named CoordinateStack that implements a stack data structure for
 * storing Coordinate objects.
 * @property {Coordinate} Root - `Root` is an array of `Coordinate` objects with a maximum size of
 * `MAX_COL * MAX_ROW`. It stores the elements of the stack.
 * @property {int} Size - The `Size` property in the `CoordinateStack` struct represents the current
 * number of elements stored in the stack. It is used to keep track of the number of elements pushed
 * onto the stack and helps in managing the stack operations like push, pop, and checking if the stack
 * is empty.
 */
...
struct Player
{
	...
	/**
	 * @brief Constructs a new Player object with optional win, tie, and lose counts.
	 * 
	 * This constructor initializes the Username and Password arrays to empty strings
	 * and sets the Win, Tie, and Lose member variables to the provided values.
	 * 
	 * @param win The initial number of wins for the player. Default is 0.
	 * @param tie The initial number of ties for the player. Default is 0.
	 * @param lose The initial number of losses for the player. Default is 0.
	 */
...
/**
 * @brief Loads account information from a file and populates the Accounts_ array.
 * 
 * This function reads account data from a file named "accounts.etai". It first reads the number of accounts
 * and ensures it does not exceed the maximum allowed accounts (MAX_ACCOUNT). Then, it reads the username,
 * password, win, tie, and lose statistics for each account and stores them in the Accounts_ array.
 * 
 * @note If the file cannot be opened, the function returns immediately without modifying any data.
 */
void loadAccounts()
{
	ifstream file;
 ...
 *
 * This function writes the account information, including username, password,
 * and game statistics (win, tie, lose), to a file named "accounts.etai".
 * The first line of the file contains the total number of accounts.
 * Each subsequent set of lines contains the information for one account.
 */
void saveAccounts()
{
	ofstream file;
